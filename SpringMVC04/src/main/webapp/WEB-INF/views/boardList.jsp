...
<script type="text/javascript">
	function goPage(page) {
		location.href = "${cpath}/boardList.do?page=" + page;
	}
</script>
	...
	<div class="container">
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<c:if test="${empty mvo}">
					<!-- mvo가 하나이기에 sessionScope.mvo에서 앞에 생략 -->
					...
					<c:forEach var="vo" items="${list}">
						<tr>
							...
							<td><c:if test="${vo.boardLevel>0}">
								...
								</c:if> <c:if test="${vo.boardLevel>0}">
									<c:if test="${vo.boardAvailable==1}">
										<a href="${cpath}/boardView.do?idx=${vo.idx}"><span
											class="bi bi-reply" style="color: red;"></span>${vo.title}</a>
									</c:if>
						...
						<tr>
							<td colspan="5">
								<button class="btn btn-sm btn-primary"
									onclick="location.href='${cpath}/boardWrite.do'">글쓰기</button>
							</td>
						</tr>
					</c:if>
					...
					<div style="text-align: center;">
						<ul class="btn-group pagination">
							...
							<c:if test="${pageMaker.prev}">
								<li><i class="fa"><a
										href="javascript:goPage(${pageMaker.startPage-1})">◀</a></i></li>
							</c:if>
							<!-- 페이징 처리 -->
							<c:forEach var="pageNum" begin="${pageMaker.startPage}"
								...
								<c:if test="${pageMaker.cri.page==pageNum}">
									<li><i class="fa"><a
											href="javascript:goPage(${pageNum})"><font color="red">${pageNum}</font></a></i></li>
								</c:if>
								<c:if test="${pageMaker.cri.page!=pageNum}">
									<li><i class="fa"><a
											href="javascript:goPage(${pageNum})">${pageNum}</a></i></li>
								</c:if>
							</c:forEach>
							<!-- 다음 페이지 -->
							<c:if test="${pageMaker.next}">
								<li><i class="fa"><a
										href="javascript:goPage(${pageMaker.endPage+1})">▶</a></i></li>
							</c:if>
						</ul>
					</div>
